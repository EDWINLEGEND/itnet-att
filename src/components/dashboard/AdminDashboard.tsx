...
import { format, startOfDay, parseISO, addWeeks } from "date-fns";
...
export function AdminDashboard({ adminUser }: AdminDashboardProps) {
  ...
  const [adminWeekView, setAdminWeekView] = useState<"this_week" | "next_week">("this_week");

  const today = useMemo(() => startOfDay(new Date()), []);
  const todayStr = useMemo(() => format(today, "yyyy-MM-dd"), [today]);

  const teamWeekDays = useMemo(() => {
    if (adminWeekView === "next_week") {
      return getTeamWeekStatus(addWeeks(today, 1));
    }
    return getTeamWeekStatus(today);
  }, [getTeamWeekStatus, adminWeekView, today]);
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="p-4 sm:p-6">
          ...
          {/* Metric Stats Cards */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 mt-5 pt-1">
            <div className="p-3.5 rounded-2xl bg-muted/30 shadow-2xs">
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5">Company average</div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-muted/30 shadow-2xs">
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5">8h shifts today</div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-muted/30 shadow-2xs">
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5">4h shifts today</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-muted/30 shadow-2xs">
              <div className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider flex items-center justify-between">
                <span>On Leave</span>
                <span className="w-2 h-2 rounded-[1.5px] bg-rose-500" />
              </div>
              <div className="text-2xl font-bold font-mono text-rose-500 mt-1">
                {todayOverview.leave}
              </div>
              <div className="text-[10px] text-muted-foreground mt-0.5">Off duty today</div>
            </div>
          </div>
        </CardContent>
      </Card>

      {/* Today's Team Attendance Watch Card */}
      <Card className="border-0 shadow-sm">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                Live daily status for all team members
              </p>
            </div>
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
            {employees.map((emp) => {
              ...
              const isLeave = shift === "leave";
              const isOnline = shift && shift !== "leave" && rec?.workLocation === "remote";
              const isOffice = shift && shift !== "leave" && (!rec?.workLocation || rec?.workLocation === "office");

              return (
                <div
                  key={emp.id}
                  className="p-3 rounded-2xl bg-muted/30 flex items-center justify-between gap-2.5 shadow-2xs"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div
                      className={`h-9 w-9 rounded-xl ${empTheme.avatarBg} flex items-center justify-center text-xs font-bold shrink-0 shadow-2xs`}
                    >
                      {emp.initials}
                    </div>
                  ...
                  <div className="shrink-0 text-right">
                    {shift ? (
                      <div className="space-y-0.5">
                        ...
                        >
                          {SHIFT_CONFIGS[shift].shortLabel}
                        </Badge>
                        <div className="text-[10px] font-medium">
                          {isLeave ? (
                            ...
                            <span className="text-zinc-700 dark:text-zinc-300">🏢 Office</span>
                          ) : null}
                        </div>
                      </div>
                    ) : (
              ...
              {allUpcomingLeaves.map((l) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="text-muted-foreground hover:text-rose-600 p-0.5 rounded cursor-pointer transition-colors font-bold ml-0.5"
                    title="Cancel leave"
                  >
                    &times;
                  </button>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="p-4 sm:p-5 pb-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="space-y-0.5">
              <CardTitle className="text-sm sm:text-base font-semibold">
                Team Schedule &mdash; {adminWeekView === "next_week" ? "Next Week" : "This Week"} Radar
              </CardTitle>
              <p className="text-xs text-muted-foreground">
                {adminWeekView === "next_week"
                  ? "Upcoming work locations & schedules for next week"
            ...
            <div className="flex items-center gap-1 bg-muted/60 p-1 rounded-xl text-xs self-start sm:self-auto">
              ...
              <button
                type="button"
                onClick={() => setAdminWeekView("next_week")}
                className={`px-3 py-1 rounded-lg font-medium cursor-pointer transition-colors ${
                  adminWeekView === "next_week"
                    ? "bg-background text-foreground shadow-2xs font-semibold"
                    : "text-muted-foreground hover:text-foreground"
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border/40">
                ...
                <TableHead className="pr-6">Total Hours</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
